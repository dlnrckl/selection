...
FILENAME = sys.argv[1]  # Path to the .ms file
RUN = sys.argv[2]       # Run number
SAMPLING_TIME = sys.argv[3]  # Sampling time
OUTPUT_DIR = sys.argv[4]     # Output directory (provided by full_pipeline.sh)
...
# Process the data
chroms = read_msms(FILENAME, 32, 50000)[0]
positions = read_msms(FILENAME, 32, 50000)[1]

# Calculate summary statistics
statistics = sum_stats(chroms, positions, 32, RUN, SAMPLING_TIME)[1]

# Save the results to the output file
with open(output_filepath, 'a') as outf:
    outf.write('\t'.join(str(element) for element in statistics) + '\n')
